...
def dfs(idx, sum_v, a, s, m, d):
    ...
    # 덧샘
    if a:
        dfs(idx + 1, sum_v + A[idx], a - 1, s, m, d)

    # 뺼셈
    if s:
        dfs(idx + 1, sum_v - A[idx], a, s - 1, m, d)

    # 곱셈
    if m:
        dfs(idx + 1, sum_v * A[idx], a, s, m - 1, d)

    # 나눗셈
    if d:
        dfs(idx + 1, int(sum_v / A[idx]), a, s, m, d - 1)
